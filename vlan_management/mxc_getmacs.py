...
def __Main__(Org):
    ...
    def __AbreCSV():
        ...
        global outputCsv
        global filename
        filename = "ReporteMAC_" + Org + ".csv"
        outputCsv = open(filename, 'wb')
        fieldnames = ['NETWORK', 'MAC', 'MACWAN1', 'SERIAL', 'IPMANAGEMENT']
    ...
    for x in range(0, array):
        ...
        #saca IP de VLAN50
        url2= 'https://dashboard.meraki.com/api/v0/networks/' + networkId + '/vlans/50'
        r2 = requests.get(url2, headers=headers)
        if r2.status_code == 200:
            r2.json()
            json_data2 = json.loads(r2.text)
            ipvlan50 = json_data2['applianceIp']
        else:
            ipvlan50 = "Null"
        #Network Name
        ...
        data = {'NETWORK': networkName, 'MAC': mac, 'MACWAN1': macwan1, 'SERIAL': serial, 'IPMANAGEMENT': ipvlan50}
